...
#### 5.3.8 Topologia de Instâncias (PaaS / Cloud)

Esta seção detalha como cada serviço da aplicação é mapeado para uma **instância de hospedagem em nuvem**, expondo os pontos de entrada, variáveis de ambiente críticas e o pipeline de entrega contínua (CI/CD):

```mermaid
graph TD
    subgraph GitHub ["☁️ GitHub — Repositório & CI/CD"]
        Repo["Repositório\nXLS_FUSION"]
        GHA["GitHub Actions\n(pipeline: build → test → deploy)"]
        Repo -->|push / merge em main| GHA
    end

    subgraph Vercel ["🟢 Vercel — Instância Frontend (PaaS)"]
        direction TB
        SPADeploy["SPA Build\n(Vite — dist/)"]
        CDN["CDN Global\n(Edge Network Vercel)"]
        EnvFront["ENV:\nVITE_API_URL=https://api.xlsfusion.app"]
        SPADeploy --> CDN
        SPADeploy -.- EnvFront
    end

    subgraph Render ["🟣 Render — Instância Backend (PaaS)"]
        direction TB
        NodeService["Web Service\nNode.js 20 LTS\nExpress 5  ·  PORT: 8080"]
        EnvBack["ENV:\nMONGO_URI=mongodb+srv://...\nJWT_SECRET=••••••\nNODE_ENV=production"]
        NodeService -.- EnvBack
    end

    subgraph Atlas ["🍃 MongoDB Atlas — Instância de Dados (DBaaS)"]
        direction TB
        M0["Cluster M0 Free Tier\n(ou M10+ para produção)\nRegião: us-east-1"]
        Replica["Replica Set\n3 nós (Primary + 2 Secondary)"]
        GridFSStore["GridFS\n(Chunks & Files collections)"]
        M0 --> Replica
        M0 --> GridFSStore
    end

    GHA -->|"Deploy automático\n(Frontend)"| Vercel
    GHA -->|"Deploy automático\n(Backend)"| Render

    CDN -->|"HTTPS 443\n(Requisições API via VITE_API_URL)"| NodeService
    NodeService -->|"mongodb+srv://\nPorta 27017 / TLS"| M0
```

| Instância | Provedor | Plano Sugerido | URL Pública | Porta Exposta |
|---|---|---|---|---|
| **Frontend SPA** | Vercel | Hobby (gratuito) / Pro | `https://xlsfusion.app` | 443 (HTTPS) |
| **Backend API** | Render | Free / Starter (512 MB RAM) | `https://api.xlsfusion.app` | 8080 → 443 via proxy |
| **Banco de Dados** | MongoDB Atlas | M0 Free / M10 Dedicado | `cluster0.xxxxx.mongodb.net` | 27017 (TLS/SRV) |
| **Storage Binário** | MongoDB Atlas | GridFS (integrado ao Atlas) | — (acesso interno via API) | — |
| **CI/CD** | GitHub Actions | Gratuito (OSS) | — | — |

*   **Separação de Domínios**: O frontend é hospedado em um domínio raiz (`xlsfusion.app`) servido pela CDN global da Vercel, enquanto o backend opera em um subdomínio dedicado (`api.xlsfusion.app`), permitindo políticas CORS específicas e escalabilidade independente de cada camada.
*   **Segredos e Configuração**: Variáveis sensíveis (`JWT_SECRET`, `MONGO_URI`, `BCRYPT_SALT_ROUNDS`) são injetadas como **Environment Variables** nas configurações de cada plataforma (Render e Vercel), nunca versionadas no repositório, seguindo o padrão *The Twelve-Factor App*.
*   **Pipeline CI/CD**: A cada *push* na branch `main`, o GitHub Actions executa as três etapas sequenciais: (1) **Build** (`npm run build`), (2) **Test** (`npm test -- --coverage`) com gate de cobertura mínima de 70%, e (3) **Deploy** automático via CLI da Vercel e webhook do Render, garantindo que apenas código validado chegue a produção.

#### 5.3.9 API REST e Rotas do Sistema

O backend do **XLS_FUSION** fornece uma API REST organizada por domínios de negócio e protegida por um middleware de autenticação JWT (`requireAuth`). As rotas mapeiam as requisições HTTP para os respectivos controladores da arquitetura MVC.

Abaixo está o detalhamento completo das rotas da API e suas respectivas funcionalidades:

| Categoria | Método | Rota | Autenticação | Descrição / Funcionalidade |
| :--- | :---: | :--- | :---: | :--- |
| **Sistema** | `GET` | `/api/health` | Livre | *Health check*: Verifica se a API está online e respondendo (retorna status 200). |
| **Autenticação** | `POST` | `/api/signup` | Livre | Registra um novo usuário no sistema, criptografando a senha com `bcrypt`. |
| **Autenticação** | `POST` | `/api/login` | Livre | Valida as credenciais do usuário e retorna o token JWT assinado. |
| **Autenticação** | `GET` | `/api/me` | 🔐 JWT | Retorna as informações de perfil do usuário atualmente autenticado. |
| **Autenticação** | `POST` | `/api/refresh` | 🔐 JWT | Atualiza e revalida a sessão com um novo token de acesso JWT. |
| **Autenticação** | `PUT` | `/api/user/profile` | 🔐 JWT | Permite ao usuário logado atualizar seus dados cadastrais. |
| **Autenticação** | `POST` | `/api/forgot-password`| Livre | Envia um fluxo/e-mail para recuperação de senha esquecida. |
| **Autenticação** | `POST` | `/api/reset-password` | Livre | Define uma nova senha para o usuário após validação de token de recuperação. |
| **Formulários** | `GET` | `/api/forms` | 🔐 JWT | Lista todos os formulários que o usuário logado possui acesso (criados por ele ou permitidos). |
| **Formulários** | `GET` | `/api/my-responses` | 🔐 JWT | Lista as respostas submetidas pelo próprio usuário autenticado. |
| **Formulários** | `POST` | `/api/forms` | 🔐 JWT | Salva ou atualiza a estrutura de um formulário (opera como *upsert*). |
| **Formulários** | `GET` | `/api/forms/:id` | 🔐 JWT | Obtém os detalhes estruturais e de configuração de um formulário específico por ID. |
| **Formulários** | `GET` | `/api/forms/:id/response`| 🔐 JWT | Recupera a resposta atual do usuário autenticado para aquele formulário específico. |
| **Formulários** | `POST` | `/api/forms/:id/response`| 🔐 JWT | Salva o rascunho ou a resposta definitiva do usuário logado para o formulário. |
| **Formulários** | `GET` | `/api/forms/:id/responses-dashboard`| 🔐 JWT | Retorna todas as respostas consolidadas de um formulário para visualização do proprietário. |
| **Métricas/Analytics**| `GET` | `/api/analytics/overview`| 🔐 JWT | Retorna indicadores consolidados de uso (KPIs) para o painel gerencial. |
| **Métricas/Analytics**| `GET` | `/api/analytics/system-metrics`| 🔐 JWT | Retorna métricas de performance do sistema (upload, uso de memória e cobertura de testes Jest). |
| **Público** | `GET` | `/api/public-forms/:id`| Livre | Carrega a estrutura de um formulário para preenchimento por visitantes externos. |
| **Público** | `GET` | `/api/public-forms/:id/responses`| Livre | Obtém rascunhos de respostas públicas previamente salvos baseados no e-mail do respondente. |
| **Público** | `POST` | `/api/public-forms/:id/responses`| Livre | Envia a submissão final das respostas públicas preenchidas pelo formulário web. |
| **Público** | `POST` | `/api/public-forms/:id/request-access`| Livre | Permite que respondentes não autorizados solicitem acesso a um formulário restrito. |
| **Pedidos de Acesso**| `POST` | `/api/forms/:formId/request-access`| 🔐 JWT | Solicita acesso a um formulário privado para usuários autenticados. |
| **Pedidos de Acesso**| `GET` | `/api/access-requests`| 🔐 JWT | Lista todos os pedidos de acesso pendentes de aprovação associados aos formulários do usuário. |
| **Pedidos de Acesso**| `POST` | `/api/forms/:formId/requests/:requestId/:action`| 🔐 JWT | Permite aprovar (`approve`) ou rejeitar (`deny`) um pedido de acesso específico. |
| **Notificações** | `GET` | `/api/notifications` | 🔐 JWT | Lista as notificações de sistema (ex: solicitações de acesso) destinadas ao usuário. |
| **Notificações** | `GET` | `/api/notifications/unread-count`| 🔐 JWT | Retorna o total de notificações não lidas. |
| **Notificações** | `POST` | `/api/notifications/mark-read`| 🔐 JWT | Marca as notificações recebidas como lidas. |
| **Arquivos (GridFS)**| `POST` | `/api/upload-anexo` | 🔐 JWT | Faz o upload de comprovante de resposta integrado ao GridFS do MongoDB (com login). |
| **Arquivos (GridFS)**| `POST` | `/api/public-upload-anexo`| Livre | Faz o upload de comprovante de resposta integrado ao GridFS do MongoDB (público). |
| **Arquivos (GridFS)**| `POST` | `/api/upload-planilha`| 🔐 JWT | Permite o upload de arquivos de planilha Excel estruturadas para o GridFS. |
| **Compartilhamento** | `POST` | `/api/share-spreadsheet`| 🔐 JWT | Cria um link público de compartilhamento de planilha a partir de um arquivo do GridFS. |
| **Compartilhamento** | `GET` | `/api/share/:hash` | Livre | Retorna as informações básicas da planilha vinculada ao hash público gerado. |
| **Compartilhamento** | `GET` | `/api/share/:hash/download`| Livre | Faz o download da planilha diretamente do GridFS por meio de fluxo de bytes (*stream*). |

> 🔐 = Requer cabeçalho HTTP `Authorization` contendo um token Bearer JWT válido.
