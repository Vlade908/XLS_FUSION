...
export default function ResponderView() {
  ...
  return (
    <div className="w-full pb-20 relative overflow-x-hidden font-sans text-slate-800 dark:text-slate-200">
      {/* Subtle background accents — z-0 so content is above */}
      <div className="pointer-events-none absolute top-0 left-0 w-full h-full overflow-hidden z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] bg-indigo-500/3 dark:bg-indigo-500/5 blur-[100px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40vw] h-[40vw] bg-purple-500/3 dark:bg-purple-500/5 blur-[100px] rounded-full" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10 pt-8">
        ...
        {viewMode === "dashboard" && (
          ...
          >
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 pb-6 border-b border-slate-200/60 dark:border-slate-800">
              <div className="text-center md:text-left space-y-1">
                <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight flex items-center justify-center md:justify-start gap-3">
                  <span className="bg-gradient-to-r from-indigo-500 via-indigo-400 to-purple-500 bg-clip-text text-transparent italic">Central de Respostas</span>
                </h1>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Responda formulários online ou preencha planilhas do Excel offline</p>
              </div>

              {/* Segmented Control */}
              <div className="bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl flex gap-1.5 border border-slate-200/80 dark:border-slate-800 shadow-sm">
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all ${
                    responderType === 'online' 
                      ? 'bg-white dark:bg-slate-950 text-indigo-600 dark:text-indigo-400 shadow-md border border-slate-200/30 dark:border-slate-800' 
            ...
            {responderType === 'online' && (
              <div className="space-y-8 animate-in fade-in duration-300">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 p-8 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-4 text-indigo-100/40 dark:text-indigo-500/15 font-black text-6xl pointer-events-none select-none italic transition-all group-hover:scale-105">#CODE</div>
                  <div className="relative z-10 max-w-xl">
                    <h2 className="text-lg font-black text-slate-800 dark:text-slate-200 uppercase italic tracking-wide mb-2">RESPONDER NOVO FORMULÁRIO</h2>
                    <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold mb-6">Insira o código de compartilhamento ou ID do formulário para iniciar.</p>
